...
class Resizing(object):
    ...
    def __call__(self, data):
        ...
        img = resize(img, (self.resizing, self.resizing), anti_aliasing=True)  # skimage.transform.resize
        mask = resize(mask, (self.resizing, self.resizing), anti_aliasing=True)  # skimage.transform.resize
...
class RandomFlip(object):
    def __call__(self, data):
        ...
        if prob <= 0.25:
            img = rotate(img, angle=rotate_list[0])
            mask = rotate(mask, angle=rotate_list[0])
        elif prob > 0.25 and prob <= 0.5 :
            img = rotate(img, angle=rotate_list[1])
            mask = rotate(mask, angle=rotate_list[1])
        elif prob > 0.5 and prob <= 0.75:
            img = rotate(img, angle=rotate_list[2])
            mask = rotate(mask, angle=rotate_list[2])
        else:
            img = rotate(img, angle=rotate_list[3])
            mask = rotate(mask, angle=rotate_list[3])
        return {'image': img, 'mask': mask}
...
class ToTensor(object):
    ...
    def __call__(self, data):
        ...
        return {'image': torch.from_numpy(img_trans),
                'mask': torch.from_numpy(mask_trans)}